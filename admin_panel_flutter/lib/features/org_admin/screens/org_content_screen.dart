...
import '../widgets/org_admin_localization_sheets.dart';
...
class OrgContentScreen extends ConsumerStatefulWidget {
  const OrgContentScreen({super.key});

  @override
  ConsumerState<OrgContentScreen> createState() => _OrgContentScreenState();
}

class _OrgContentScreenState extends ConsumerState<OrgContentScreen>
...
class _TheoryTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final chaptersAsync = ref.watch(orgTheoryProvider);
        ...
        columns: [
          ...
          AppTableColumn(
            ...
            cellBuilder: (c) => _RowActions(
              onEdit:     () => _showDialog(context, ref, c),
              onLocalize: () => showOrgTheoryChapterLocalizationsSheet(
                context:     context,
                chapterId:   c.id!,
                parentLabel: c.title,
  ...
  Future<void> _confirmDelete(
    ...
    if (!ok) return;
    await ref.read(clientProvider).organizationAdmin.deleteOrgTheoryChapter(c.id!);
...
class _TrainingTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final paramsAsync = ref.watch(orgTrainingProvider);
        ...
        columns: [
          ...
          AppTableColumn(
            ...
            cellBuilder: (p) => _RowActions(
              onEdit:     () => _showDialog(context, ref, p),
              onLocalize: () => showOrgTrainingParameterLocalizationsSheet(
                context:       context,
                parameterId:   p.id!,
                parentLabel:   p.name,
      ...
      builder: (_) => _ParamDialog<TrainingParameter>(
        ...
        onSave: (param) async {
          await ref.read(clientProvider).organizationAdmin
              .upsertOrgTrainingParameter(param as TrainingParameter);
          ref.invalidate(orgTrainingProvider);
        },
      ),
    );
  }
  ...
  Future<void> _confirmDelete(
    ...
    if (!ok) return;
    await ref.read(clientProvider).organizationAdmin.deleteOrgTrainingParameter(p.id!);
    ref.invalidate(orgTrainingProvider);
  }
}
...
class _AssessmentTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final paramsAsync = ref.watch(orgAssessmentProvider);
    final params      = paramsAsync.value ?? [];
      ...
      child: AppDataTable<AssessmentParameter>(
        ...
        columns: [
          ...
          AppTableColumn(
            ...
            cellBuilder: (p) => _RowActions(
              onEdit:     () => _showDialog(context, ref, p),
              onLocalize: () => showOrgAssessmentParameterLocalizationsSheet(
                context:       context,
                parameterId:   p.id!,
                parentLabel:   p.name,
                feedbackCount: p.scoringRules.length,
  ...
  void _showDialog(
    ...
    showDialog(
      ...
      builder: (_) => _ParamDialog(
        ...
        onSave: (param) async {
          await ref.read(clientProvider).organizationAdmin
              .upsertOrgAssessmentParameter(param as AssessmentParameter);
          ref.invalidate(orgAssessmentProvider);
        },
      ),
    );
  }
  ...
  Future<void> _confirmDelete(
    ...
    final ok = await _showDeleteDialog(ctx, p.name);
    if (!ok) return;
    await ref.read(clientProvider).organizationAdmin.deleteOrgAssessmentParameter(p.id!);
    ref.invalidate(orgAssessmentProvider);
  }
}
...
class _ChapterDialogState extends ConsumerState<_ChapterDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(clientProvider).organizationAdmin.upsertOrgTheoryChapter(chapter);
      ref.invalidate(orgTheoryProvider);
                ...
                children: [
                  ...
                  if (!isLoading && count != null)
                    Padding(
                      ...
                      child: Text('$count total', style: AppTextStyles.bodyXs),
                    ),
                  AppGradientButton(
                    label:     'Add',
